package shop.bluebooktle.backend.book_order.handler;
...
@RestControllerAdvice(basePackages = "shop.bluebooktle.backend.book_order.controller")
@Slf4j
public class BookOrderGlobalExceptionHandler {
	...
	protected ResponseEntity<JsendResponse<?>> handleGenericException(Exception ex) {
		...
		JsendResponse<?> response = JsendResponse.error(
			"도서 주문 서비스에서 예상치 못한 오류가 발생했습니다. 문제가 지속되면 관리자에게 문의하세요.",
			errorCode.getCode()
		);
