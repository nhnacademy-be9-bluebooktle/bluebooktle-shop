...
public interface PackagingOptionRepository extends JpaRepository<PackagingOption, Long> {
	// 삭제되지 않은 포장 전체 조회
	@Query("SELECT p FROM PackagingOption p WHERE p.deletedAt IS NULL")
	List<PackagingOption> findAllAvailable();

	// 삭제되지 않은 개별 포장 조회
	@Query("SELECT p FROM PackagingOption p WHERE p.id = :id AND p.deletedAt IS NULL")
