...
public class AdminMembershipLevelController {
	...
	public String listMembershipLevels(Model model, HttpServletRequest request) {
		...
		// 할 일: adminMembershipLevelService.getAllLevels() 호출 (minNetSpent 순으로 정렬)
	...
	public String membershipLevelForm(@PathVariable(value = "levelId", required = false) Long levelId,
		...
		if (levelId != null) {
			...
			// 할 일 : adminMembershipLevelService.getLevelById(levelId) 호출
	...
	public String saveMembershipLevel(//@Valid
		...
		// 할 일 : 유효성 검사 (등급명 필수, 금액 음수 불가, min < max 논리 검사 등)
		...
		// 할 일 : 다른 등급과의 min/max 금액 구간 중첩 검사 필요
		...
		try {
			// 할 일 : 실제 서비스 로직 호출 (adminMembershipLevelService.saveLevel(levelDto))
			if (levelDto.getId() == null) {
	...
	public String deleteMembershipLevel(
		@PathVariable Long levelId,
		RedirectAttributes redirectAttributes) {
		log.info("회원 등급 정책 삭제 요청: ID {}", levelId);
		// 할 일 : 해당 등급에 속한 회원이 있는지 확인하는 로직 필요. 있다면 삭제 불가 또는 다른 등급으로 이전 정책 필요.
